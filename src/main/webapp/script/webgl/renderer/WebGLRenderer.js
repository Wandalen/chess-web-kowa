...
THREE.WebGLRenderer = function ( parameters ) {
  ...
  */
  ...
  this.render = function ( scene, camera, renderTarget, forceClear ) {
    ...
    for ( i = 0, il = renderList.length; i < il; i ++ ) {
      ...
      if ( object.visible ) {
        ...
        this.unrollImmediateBufferMaterial( webglObject );
    ...
    if ( scene.overrideMaterial ) {
      ...
      this.renderObjects( scene.__webglObjects, false, "", camera, lights, fog, true, material );
      this.renderObjectsImmediate( scene.__webglObjectsImmediate, "", camera, lights, fog, false, material );
  ...
  this.renderObjectsImmediate = function( renderList, materialType, camera, lights, fog, useBlending, overrideMaterial ) {
...
*/
  ...
  this.initWebGLObjects = function ( scene ) {
    ...
    for ( var o = scene.__webglObjects.length-1, ol = scene.__webglObjects.length; o >= 0; o -- ) {
      ...
      if ( object.__webglInit === undefined ) {
        ...
        //console.log( 'updating', o, object.name, object.id );
      ...
      {
        //console.log( 'not updating', o, object.name, object.id );
      }
  ...
  this.addObject = function( object, scene ) {
    ...
    if ( object.__webglInit === undefined ) {
      ...
      object._projectionMatrixInverse = new THREE.Matrix3();
  ...
  this.invalidateObject_ = function ( object, scene ) {
  ...
  removeObject = this.invalidateObject_;
...
*/
  ...
  function loadUniformsMatrices ( uniforms, object, camera ) {
    ...
    if ( uniforms.projectionMatrixInverse ) {

      //object._projectionMatrixInverse.getInverse( camera.projectionMatrix );
      object._projectionMatrixInverse.getNormalMatrix( camera.projectionMatrix );
      _gl.uniformMatrix3fv( uniforms.projectionMatrixInverse, false, object._projectionMatrixInverse.elements );
  ...
  this.updateRenderTargetMipmap = function( renderTarget ) {
    ...
    if ( !renderTarget || !renderTarget.generateMipmaps || renderTarget.minFilter === THREE.NearestFilter || renderTarget.minFilter === THREE.LinearFilter ) return 0;
...
//THREE.WebGLRenderer.prototype.renderWithMaterial = function ( scene, camera, renderTarget, forceClear, materialName ) {
  this.renderWithMaterial = function ( scene, camera, renderTarget, forceClear, materialName ) {
    ...
    if( !materialName ) renderPlugins( this.renderPluginsPre, scene, camera );
    ...
    if ( scene.overrideMaterial ) {
      ...
      this.renderObjects( scene.__webglObjects, true, "select", camera, lights, null, false, material );
      this.renderObjectsImmediate( scene.__webglObjectsImmediate, "select", camera, lights, null, false, material );
  ...
  for ( var i = start; i !== end; i += delta ) {
    ...
    if ( webglObject.render ) {
      ...
      if ( overrideMaterial ) {
        ...
        {
          this.setColorWithObject( material.color, object );
          _currentProgram = -1;
        }
