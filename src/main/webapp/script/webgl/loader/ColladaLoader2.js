...
{
  this._init()
  ColladaLoader2.verbosity = ColladaLoader2.messageError;
  //console.log( 'ColladaLoader2.verbosity: ',ColladaLoader2.verbosity );
};
        ...
        switch (child.nodeName) {
          case "COLLADA":
            if (colladaFound) {
              ...
              if( !this.dae.scena )
                console.log( 'Warning: ColladaLoader2.File: no scena' );
            }
            break;
          default:
...
ColladaLoader2.File.prototype._parseCollada = function(el) {
  ...
  _ref = el.childNodes;
  if( !_ref.length ) console.log( 'Warning: ColladaLoader2.File: no children' );
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    child = _ref[_i];
    if (child.nodeType === 1) {
      switch (child.nodeName) {
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "unit":
          ...
          break;
        case "up_axis":
          this.dae.asset.upAxis = ( child.textContent || child.text || '' ).toUpperCase().charAt(0);
          break;
        case "contributor":
        case "created":
        case "modified":
...
ColladaLoader2.File.prototype._parseInstanceController = function(parent, el) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "skeleton":
          controller.skeletons.push(new ColladaLoader2.urlLink(( child.textContent || child.text || '' ), this));
          break;
        case "bind_material":
...
ColladaLoader2.File.prototype._parseTransformElement = function(parent, el) {
  ...
  transform.data = ColladaLoader2._strToFloats(( el.textContent || el.text ));
...
ColladaLoader2.File.prototype._parseEffectNewparam = function(scope, el) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "semantic":
          param.semantic = ( child.textContent || child.text || '' );
          break;
        case "float":
          param.floats = ColladaLoader2._strToFloats(( child.textContent || child.text || '' ));
          break;
        case "float2":
          param.floats = ColladaLoader2._strToFloats(( child.textContent || child.text || '' ));
          break;
...
ColladaLoader2.File.prototype._parseEffectSurface = function(scope, el) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "init_from":
          surface.initFrom = new ColladaLoader2.urlLink(( child.textContent || child.text || '' ), this);
          break;
        case "format":
          surface.format = ( child.textContent || child.text || '' );
          break;
        case "size":
          surface.size = ColladaLoader2._strToFloats(( child.textContent || child.text || '' ));
          break;
        case "viewport_ratio":
          surface.viewportRatio = ColladaLoader2._strToFloats(( child.textContent || child.text || '' ));
          break;
        case "mip_levels":
          surface.mipLevels = parseInt(( child.textContent || child.text || '' ), 10);
          break;
        case "mipmap_generate":
          surface.mipmapGenerate = ( child.textContent || child.text || '' );
          break;
        default:
          ColladaLoader2._reportUnexpectedChild(child);
      }
...
ColladaLoader2.File.prototype._parseEffectSampler = function(scope, el) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "source":
          sampler.surface = new ColladaLoader2.FxLink(( child.textContent || child.text || '' ), scope, this);
          break;
        case "instance_image":
          sampler.image = this._getAttributeAsUrlLink(child, "url", true);
          break;
        case "wrap_s":
          sampler.wrapS = ( child.textContent || child.text || '' );
          break;
        case "wrap_t":
          sampler.wrapT = ( child.textContent || child.text || '' );
          break;
        case "minfilter":
          sampler.minfilter = ( child.textContent || child.text || '' );
          break;
        case "magfilter":
          sampler.magfilter = ( child.textContent || child.text || '' );
          break;
        case "border_color":
          sampler.borderColor = ColladaLoader2._strToFloats(( child.textContent || child.text || '' ));
          break;
        case "mipmap_maxlevel":
          sampler.mipmapMaxLevel = parseInt(( child.textContent || child.text || '' ), 10);
          break;
        case "mipmap_bias":
          sampler.mipmapBias = parseFloat(( child.textContent || child.text || '' ));
          break;
        default:
          ColladaLoader2._reportUnexpectedChild(child);
      }
...
var xmlGetChildNode = function( child )
{
  if( child.childNodes.length > 1 )
  {
    return ( child.childNodes[1].textContent !== undefined ) ? child.childNodes[1].textContent : child.childNodes[1].text;
  }
  else if( child.childNodes.length == 1 )
  {
    return ( child.childNodes[0].textContent !== undefined ) ? child.childNodes[0].textContent : child.childNodes[0].text;
  }
  else
  {
    return ( child.childNodes.textContent !== undefined ) ? child.childNodes.textContent : child.childNodes.text;
  }
}
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        ...
        case "bump":
          ...
          break;
        case "shininess":
          technique.shininess = parseFloat( xmlGetChildNode( child ) );
          break;
        case "reflectivity":
          technique.reflectivity = parseFloat( xmlGetChildNode( child ) );
          break;
        case "transparency":
          technique.transparency = parseFloat( xmlGetChildNode( child ) );
          break;
        case "index_of_refraction":
          technique.index_of_refraction = parseFloat( xmlGetChildNode( child ) );
          break;
        case "double_sided":
          technique.double_sided = (parseFloat(( child.textContent || child.text || '' ))) > 0;
          break;
        default:
          if (profile === "COMMON") {
            ColladaLoader2._reportUnexpectedChild(child);
...
ColladaLoader2.File.prototype._parseEffectColorOrTexture = function(technique, el) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "color":
          colorOrTexture.color = ColladaLoader2._strToColor(( child.textContent || child.text || '' ));
          break;
        case "texture":
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "bool_array":
          ...
          source.data = ColladaLoader2._strToBools(( child.textContent || child.text || '' ));
          break;
        case "float_array":
          source.sourceId = this._getAttributeAsString(child, "id", null, false);
          source.data = ColladaLoader2._strToFloats(( child.textContent || child.text || '' ));
          break;
        case "int_array":
          source.sourceId = this._getAttributeAsString(child, "id", null, false);
          source.data = ColladaLoader2._strToInts(( child.textContent || child.text || '' ));
          break;
        case "IDREF_array":
        case "Name_array":
          source.sourceId = this._getAttributeAsString(child, "id", null, false);
          source.data = ColladaLoader2._strToStrings(( child.textContent || child.text || '' ));
          break;
        case "technique_common":
...
ColladaLoader2.File.prototype._parseVertices = function(geometry, el) {
  ...
  //console.log( 'vertices.name: ',vertices.name );
...
ColladaLoader2.File.prototype._parseTriangles = function(geometry, el) {
  ...
  //console.log( 'triangles.name: ',triangles.name );
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "input":
          ...
          break;
        case "vcount":
          triangles.vcount = ColladaLoader2._strToInts(( child.textContent || child.text || '' ));
          break;
        case "p":
          triangles.indices = ColladaLoader2._strToInts(( child.textContent || child.text || '' ));
          break;
        default:
          ColladaLoader2._reportUnexpectedChild(child);
      }
...
ColladaLoader2.File.prototype._parseImage = function(el) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "init_from":
          image.initFrom = ( child.textContent || child.text || '' );
          break;
        default:
          ColladaLoader2._reportUnexpectedChild(child);
      }
...
ColladaLoader2.File.prototype._parseBindShapeMatrix = function(parent, el) {
  parent.bindShapeMatrix = ColladaLoader2._strToFloats(( el.textContent || el.text ));
};
    ...
    if (child.nodeType === 1) {
      switch (child.nodeName) {
        case "input":
          ...
          break;
        case "vcount":
          weights.vcount = ColladaLoader2._strToInts(( child.textContent || child.text || '' ));
          break;
        case "v":
          weights.v = ColladaLoader2._strToInts(( child.textContent || child.text || '' ));
          break;
        default:
          ColladaLoader2._reportUnexpectedChild(child);
      }
...
ColladaLoader2.File.prototype._parseLightColor = function(el, profile, light) {
  light.color = ColladaLoader2._strToFloats(( el.textContent || el.text ));
};
...
ColladaLoader2.File.prototype._parseLightParam = function(el, profile, light) {
  ...
  param.value = parseFloat(( el.textContent || el.text ));
};
...
ColladaLoader2.File.prototype._addDefaultNodeProperties = function(daeNode, threejsParent, threejsNode) {

  threejsNode.name = daeNode.name != null ? daeNode.name : "";
  threejsNode.key = daeNode.id != null ? daeNode.id : "";
...
ColladaLoader2.File.prototype._createSceneGraphNode = function(daeNode, threejsParent) {
  ...
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    if (threejsMesh != null) {
      ...
      this._addDefaultNodeProperties( daeGeometry, threejsParent,threejsMesh );
  ...
  for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
    ...
    if (threejsMesh != null) {
      //threejsMesh.name = daeNode.name != null ? daeNode.name : "";
      this._addDefaultNodeProperties( daeController, threejsParent,threejsMesh );
      threejsChildren.push(threejsMesh);
    }
  }
  ...
  for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
    ...
    if (threejsLight != null) {
      ...
      this._addDefaultNodeProperties( daeLight, threejsParent,threejsLight );
  ...
  for (_l = 0, _len3 = _ref3.length; _l < _len3; _l++) {
    ...
    if (threejsCamera != null) {
      ...
      this._addDefaultNodeProperties( daeCamera, threejsParent,threejsCamera );
      threejsChildren.push(threejsCamera);
    }
  }
  if (threejsChildren.length > 1) {
    threejsNode = new THREE.Object3D();
    //this._addDefaultNodeProperties( daeNode, threejsParent,threejsNode );
...
ColladaLoader2.File.prototype._createStaticMesh = function(daeInstanceGeometry) {
  ...
  if( threejsMaterial instanceof THREE.LineBasicMaterial )
  mesh = new THREE.Line( threejsGeometry, threejsMaterial );
  else mesh = new THREE.Mesh( threejsGeometry, threejsMaterial );

  //Gutils.log( Gutils.nodeDump( mesh ) );
...
ColladaLoader2.File.prototype._createGeometryAndMaterial = function(daeGeometry, daeInstanceMaterials) {
  ...
  // xxx
  if( daeGeometry && daeGeometry.triangles && daeGeometry.triangles[0] && daeGeometry.triangles[0].type == 'linestrips' )
  {
    threejsMaterials.materials[0] = new THREE.LineBasicMaterial( { linewidth: 20, color: 0x333333, transparent: true } );
  }
  // xxx
...
ColladaLoader2.File.prototype._addTrianglesToGeometry = function(daeGeometry, triangles, materialIndex, threejsGeometry) {
  ...
  if (triangles.type !== "triangles" && triangles.type !== "linestrips") {
...
ColladaLoader2.File.prototype._addTrianglesToGeometry = function(daeGeometry, triangles, materialIndex, threejsGeometry) {
  ...
  if (triangles.type !== "triangles") {
    if( !triangles.vcount )
    ColladaLoader2._log("Geometry " + daeGeometry.id + " has non-triangle polygons", ColladaLoader2.messageError);
    else
    {
      vcount = triangles.vcount;
      for (_l = 0, _len3 = vcount.length; _l < _len3; _l++) {
...
//
  indices = triangles.indices;
  var c = (triangles.type == 'linestrips') ? Math.ceil( indices.length / 3 ) : triangles.count; // !!!
  triangleStride = Math.ceil( indices.length / c ); // !!!
  vertexStride = Math.ceil( triangleStride / 3 ) ;

//
  for (triangleIndex = _m = 0, _ref3 = c - 1; _m <= _ref3; triangleIndex = _m += 1) {
    triangleBaseOffset = triangleIndex * triangleStride;
    baseOffset0 = triangleBaseOffset + 0 * vertexStride;
    baseOffset1 = triangleBaseOffset + 1 * vertexStride;
    baseOffset2 = triangleBaseOffset + 2 * vertexStride;
...
ColladaLoader2.File.prototype._createVector3Array = function(source) {
  ...
  for (i = _i = i0; _i <= i1; i = _i += 3) {
    if( srcData[i + 2] === undefined ) break;
...
ColladaLoader2.prototype.load = function(url, readyCallback, progressCallback) {
  ...
  {
      if( isIe() && (typeof booleanValue) == 'string' )
      {
    ...
    req.onreadystatechange = function() {
      if (req.readyState === 4) {
        if (req.status === 0 || req.status === 200) {
          var response = parseXml( req.responseXML || req.response || req.responseText );
          if (response) {
            return _this.parse(response, readyCallback, url);
          } else {
...
ColladaLoader2._colladaLogConsole = function(msg, type) {
  //if( type >= 3 ) // verbosity !!!
  //console.log( 'this.verbosity: ',this.verbosity );
  //console.log( 'ColladaLoader2.verbosity: ',ColladaLoader2.verbosity );
  if( type >= ColladaLoader2.verbosity )
...
ColladaLoader2._reportInvalidTargetType = function(link, type) {
  ColladaLoader2._log("Link " + link  + " does not point to a " + type.name, ColladaLoader2.messageError);
};
